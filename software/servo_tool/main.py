...
class MainWindow(QtWidgets.QWidget, Ui_Form):
    ...
    def horizontalSlider_valuechange(self, name):
        ...
        if name == 'servoMove':
            ...
            set_servo_pulse(self.id, int(self.servoMove), 0)
        if name == 'servoID':
    ...
    def pushbutton_clicked(self, name):
        if name == 'read':
            try:
                ...
                if self.id is None:
                    ...
                    return
                self.readOrNot = True
                self.dev = get_servo_deviation(self.id)
                if self.dev is None:
                    if self.chinese:
                ...
                self.servoTemp = get_servo_temp_limit(self.id)
                (self.servoMin, self.servoMax) = get_servo_angle_limit(self.id)
                (self.servoMinV, self.servoMaxV) = get_servo_vin_limit(self.id)
                self.servoMove = get_servo_pulse(self.id)
                
                currentVin = get_servo_vin(self.id)

                currentTemp = get_servo_temp(self.id)
        ...
        if name == 'set':
            ...
            try:
                set_servo_id(self.id, ID)
                time.sleep(0.01)
                self.dev = get_servo_deviation(ID)
                if self.dev is None:
                    if self.chinese:
                    ...
                    return
                set_servo_deviation(ID, dev)
                time.sleep(0.01)
                save_servo_deviation(ID)
                time.sleep(0.01)
                d = get_servo_deviation(ID)
                if d > 125:
                    d = -(0xff-(d - 1))               
                if d != dev:
                    if self.chinese:
                        ...
                        self.message_From('Failed！')
                    return            
                set_servo_temp_limit(ID, temp)
                time.sleep(0.01)
                if get_servo_temp_limit(ID) != temp:
                    if self.chinese:
                    ...
                    return 
                set_servo_angle_limit(ID, pos_min, pos_max)
                time.sleep(0.01)
                if get_servo_angle_limit(ID) != (pos_min, pos_max):
                    if self.chinese:
                        self.message_From('角度范围设置失败！')
                    else:
                        self.message_From('Failed！')
                    return 
                set_servo_vin_limit(ID, vin_min*100, vin_max*100)
                time.sleep(0.01)
                if get_servo_vin_limit(ID) != (vin_min*100, vin_max*100):
                    if self.chinese:
                        self.message_From('电压范围设置失败！')
                    else:
                        self.message_From('Failed！')
                    return 
                set_servo_pulse(ID, pos, 0)
            except BaseException as e:
                print('error2:', e)
                if self.chinese:
            ...
            try:
                ...
                if get_servo_id() != 1:
                    if self.chinese:
                        ...
                        self.message_From('Failed！')
                    return
                set_servo_deviation(1, 0)
                time.sleep(0.01)
                save_servo_deviation(1)
                time.sleep(0.01)
                if get_servo_deviation(1) != 0:
                    if self.chinese:
                        self.message_From('偏差设置失败！')
                    else:
                        self.message_From('Failed！')
                    return
                set_servo_temp_limit(1, 85)
                time.sleep(0.01)
                if get_servo_temp_limit(1) != 85:
                    if self.chinese:
                        self.message_From('温度设置失败！')
                    else:
                        self.message_From('Failed！')
                    return
                set_servo_angle_limit(1, 0, 1000)
                time.sleep(0.01)
                if get_servo_angle_limit(1) != (0, 1000):
                    if self.chinese:
                        self.message_From('角度范围设置失败！')
                    else:
                        self.message_From('Failed！')
                    return          
                set_servo_vin_limit(1, 4500, 14000)
                time.sleep(0.01)
                if get_servo_vin_limit(1) != (4500, 14000):
                    if self.chinese:
                        self.message_From('电压范围设置失败！')
                    else:
                        self.message_From('Failed！')
                    return             
                set_servo_pulse(1, 500, 0)
            except BaseException as e:
                print('error1:', e)
                if self.chinese:
                    self.message_From('设置超时!')
